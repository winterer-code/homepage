...
(function () {
  ...
  var CONSULT = {
    /* 전송 방식
       "send.php"  → PHP 호스팅에서 메일 발송 (기본, 권장)
  ...
  function diagnose(res) {
    ...
    if (res.status === 404) return "서버에 send.php 파일이 없습니다.";
    if (res.status === 403) return "서버가 send.php 접근을 막고 있습니다.";
    if (res.status === 405 || res.status === 501) return "서버에서 PHP가 실행되지 않습니다.";
    if (res.status >= 500) return "서버에서 오류가 발생했습니다.";
    if (res.ok && /<\?php|<\?=/.test(res.text || "")) return "서버에서 PHP가 실행되지 않습니다.";
    if (res.ok) return "서버 응답을 읽을 수 없습니다.";
    return "전송에 실패했습니다.";
  }
  function techHint(res) {
    if (res.status === 404) return "send.php 를 index.html 과 같은 폴더에 올렸는지 확인하세요.";
    if (res.status === 405 || res.status === 501 || (res.ok && /<\?php|<\?=/.test(res.text || "")))
      return "PHP 를 지원하는 호스팅(리눅스 상품)이 필요합니다.";
    if (res.json && res.json.error) return "서버 응답 " + res.status + " · " + CONSULT.endpoint;
    return "서버 응답 " + res.status + " · " + CONSULT.endpoint;
  }
  ...
  Array.prototype.forEach.call(document.querySelectorAll("form[data-consult-form]"), function (form) {
    ...
    /* 메일 앱으로 보내는 방식 — 오류가 아니라 정상 절차로 안내합니다. */
    function showMailStep(d) {
      ...
      failBox.classList.add("form-fail--info");
      var p = failBox.querySelector("p");
      if (p) {
        p.innerHTML = "<b>작성이 끝났습니다.</b><br>아래 버튼을 누르면 메일 앱이 열리고, " +
                      "작성하신 내용이 그대로 담겨 있습니다. 그대로 보내주시면 접수됩니다.";
      }
      var h = failBox.querySelector("[data-form-hint]");
      if (h) { h.hidden = true; }
    ...
    function showFail(d, reason, hint) {
      if (doneBox) doneBox.hidden = true;
      if (failBox) failBox.classList.remove("form-fail--info");
      if (!failBox) { window.location.href = mailtoUrl(d); return; }
      if (mailLink) mailLink.setAttribute("href", mailtoUrl(d));
      var p = failBox.querySelector("p");
      if (p && reason) {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (location.protocol === "file:") {
        showFail(data, "미리보기 파일에서는 자동 전송이 되지 않습니다.",
                 "실제 서버에 올리면 자동으로 접수됩니다.");
        return;
      }
      /* 서버(PHP) 없이 운영하는 경우 — 메일 앱으로 보내는 것이 정상 흐름입니다. */
      if (!CONSULT.endpoint) {
        showMailStep(data);
        return;
      }
      ...
      fetch(CONSULT.endpoint, {
        ...
        .then(function (res) {
          /* 서버가 {"ok":true} 를 돌려줬을 때만 성공으로 처리합니다.
             PHP 가 실행되지 않아 소스가 그대로 돌아오는 경우를
             성공으로 오인하지 않기 위해서입니다. */
          if (res.json && res.json.ok === true) { done(); showDone(); return; }
          done();
          showFail(data, diagnose(res), techHint(res));
        })
        .catch(function () {
          done();
          showFail(data, "서버와 연결하지 못했습니다.",
                   "인터넷 연결 또는 " + CONSULT.endpoint + " 주소를 확인해 주세요.");
        });
    });
  });
